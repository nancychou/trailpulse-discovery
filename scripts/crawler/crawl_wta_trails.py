#!/usr/bin/env python3
"""TrailPulse: WTA Snoqualmie Trail Crawler (robots.txt-friendly)
...
- Adds a clean, 2-stage pipeline:
  ...
  (B) Normalize + enrich (distance/elevation/route_type/surface/water/parking_status/tags/cell/crowd/max_grade)
- Adds stronger parsing for "feet" as well as "ft"
- Adds description extraction + cleanup (removes "Print Email" noise)
- Adds Snoqualmie-area filter (toggleable) and de-dupe by stable slug id
...
Usage
  python crawl_wta_snoqualmie_trails_enhanced.py --limit 100 --out trails
...
def parse_max_grade_percent(text: str) -> Optional[int]:
    t = _lower(text)
    if not t:
        return None
    m = re.search(r"(\d{1,2})\s*%\s*(?:grade|slope)?", t)
...
def enrich_record(rec: TrailRecord) -> TrailRecord:
    ...
    if not rec.max_grade:
        rec.max_grade = parse_max_grade_percent(desc)
...
class WTATrailCrawler:
    def __init__(
        ...
        *,
        ...
        # "Snoqualmie area only" heuristic filter.
        self.snoqualmie_keywords = [
            "snoqualmie",
            "snoqualmie pass",
            "i-90",
            "north bend",
    ...
    def is_snoqualmie(self, rec: TrailRecord) -> bool:
        return True
        # if not self.snoqualmie_only:
        #     return True
        # hay = " ".join([rec.name or "", rec.description or ""]).lower()
        # return any(k in hay for k in self.snoqualmie_keywords)
    ...
    def crawl(self, limit: int = 100) -> List[TrailRecord]:
        ...
        for idx, url in enumerate(seed_urls, start=1):
            ...
            if rec and self.is_snoqualmie(rec):
                ...
                if rec:
                    print(f"  - Skipped (not Snoqualmie): {rec.name}")
...
Examples:
  ...
  python %(prog)s --debug --start-page 1 --list-pages 2 --limit 10
    ...
    ap.add_argument("--out", type=str, default="wta_snoqualmie_trails", help="Output basename (no extension)")
    ...
    ap.add_argument("--all", action="store_true", help="Disable Snoqualmie-only filter")
    ...
    print(f"  Filter: {'All trails' if args.all else 'Snoqualmie area only'}")
